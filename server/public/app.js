// FlyLock Assessment Portal - Student Assessment Script
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function checkStudentAuth() {
    try {
      const res = await fetch('/api/v1/auth/student-me');
  ...
  window.handleStudentGoogleCredentialResponse = async function(response) {
    ...
    try {
      const res = await fetch('/api/v1/auth/student-google', {
        method: 'POST',
  ...
  async function verifyExamAccess() {
    ...
    if (emailParam && emailParam.endsWith('@bitsathy.ac.in') && (!state.student || state.student.email !== emailParam)) {
      try {
        const loginRes = await fetch('/api/v1/auth/student-login', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  function startHeartbeat() {
    ...
    state.heartbeatInterval = setInterval(async () => {
      ...
      try {
        await fetch(`/api/v1/assessments/${state.currentExamAttempt.exam_code}/heartbeat`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  async function executeSubmit() {
    if (!state.currentExamAttempt) return;
    try {
      const res = await fetch(`/api/v1/assessments/${state.currentExamAttempt.exam_code}/submit`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ answers: state.userAnswers })
      });
