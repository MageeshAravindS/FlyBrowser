// FlyLock Assessment Portal - Creator & Admin Portal Logic
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadAssessmentsListForCsv() {
    try {
      const res = await fetch('/api/v1/assessments/list');
      if (!res.ok) return;
      const data = await res.json();
  ...
  btnConfirmCsvImport.addEventListener('click', async () => {
    ...
    if (isNewMode) {
      ...
      try {
        const res = await fetch('/api/v1/assessments/import-csv-new', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            examCode,
      ...
      try {
        const res = await fetch(`/api/v1/assessments/${assessmentId}/import-csv`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    try {
      ...
      tablePublishedAssessmentsBody.querySelectorAll('.btn-toggle-active').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          try {
            const res = await fetch(`/api/v1/assessments/${assId}/update`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
      ...
      tablePublishedAssessmentsBody.querySelectorAll('.btn-delete-assessment').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          if (confirm(`Are you sure you want to REVOKE and DELETE assessment '${title}' (PIN: ${code})? This will delete all questions and student attempts.`)) {
            try {
              const res = await fetch(`/api/v1/assessments/${assId}/delete`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' }
              });
              const data = await res.json();
  ...
  async function loadStudentResponses() {
    try {
      const res = await fetch('/api/v1/assessments/responses');
      if (!res.ok) {
    ...
    tableStudentResponsesBody.querySelectorAll('.btn-grant-reattempt').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (confirm(`Grant re-attempt for ${email} on assessment PIN ${code}?\n\nThis will reset their session state so they can log back in and retake the assessment.`)) {
          try {
            const res = await fetch('/api/v1/assessments/attempts/reset', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
  ...
  async function openResponseDetailModal(attemptId) {
    ...
    try {
      const res = await fetch(`/api/v1/assessments/responses/${attemptId}`);
      if (!res.ok) {
  ...
  async function loadAdminAttempts() {
    try {
      const res = await fetch('/api/v1/admin/attempts');
      if (!res.ok) return;
      const data = await res.json();
  ...
  async function resetAttempt(attemptId) {
    try {
      const res = await fetch('/api/v1/admin/attempts/reset', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  simBtnFetchToken.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/api/v1/sessions/launch', {
        method: 'POST',
        headers: {
  ...
  simBtnTestReplay.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(`/assessment/verify?token=${encodeURIComponent(state.simulatedToken)}&code=${encodeURIComponent(state.simulatedExamCode)}`);
      const data = await res.json();
      if (!res.ok) {
